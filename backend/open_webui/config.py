...
class AppConfig:
    ...
    def __setattr__(self, key, value):
        if isinstance(value, PersistentConfig):
            ...
            if self._redis:
                redis_key = f"loop-chat:config:{key}"
...
if CUSTOM_NAME:
    try:
        r = requests.get(f"https://api.chat.dfveloper.com/api/v1/custom/{CUSTOM_NAME}")
        data = r.json()
        if r.ok:
            if "logo" in data:
                WEBUI_FAVICON_URL = url = (
                    f"https://api.chat.dfveloper.com{data['logo']}"
                    if data["logo"][0] == "/"
                    else data["logo"]
                )
            ...
            if "splash" in data:
                url = (
                    f"https://api.chat.dfveloper.com{data['splash']}"
                    if data["splash"][0] == "/"
                    else data["splash"]
                )
...
if ENV == "prod":
    if OLLAMA_BASE_URL == "/ollama" and not K8S_FLAG:
        if USE_OLLAMA_DOCKER.lower() == "true":
            # if you use all-in-one docker container (LOOP Chat + Ollama)
            # with the docker build arg USE_OLLAMA=true (--build-arg="USE_OLLAMA=true") this only works with http://localhost:11434
            OLLAMA_BASE_URL = "http://localhost:11434"
        else:
            OLLAMA_BASE_URL = "http://host.docker.internal:11434"
    elif K8S_FLAG:
        OLLAMA_BASE_URL = "http://ollama-service.loop-chat.svc.cluster.local:11434"
...
PINECONE_INDEX_NAME = os.getenv("PINECONE_INDEX_NAME", "loop-chat-index")
